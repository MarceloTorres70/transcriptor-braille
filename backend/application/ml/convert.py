...
_DEFAULT_MAP = Path(__file__).resolve().parent / "braille_map.json"


def convert_to_braille_unicode(
    str_input: str, path: Optional[str] = None
) -> str:
    map_path = Path(path) if path else _DEFAULT_MAP
    with open(map_path, "r", encoding="utf-8") as fl:
        data = json.load(fl)
